...
As mentioned above you should give the label name of the controls and ensure that it is the same as the label name of the UI as it is case-sensitive. </ul>

### 3. Assert:
<ul>In this step you will assert the value obtained with the actual and return whether the test is passed or failed. This poly API available in the SLL Drona palette provides a set of assert VIs that help you in asserting the values. </ul>
...
<ul>As same as the stimulus a valid control path has to be given. There is a set of Assert VIs available in the SLL Drona palette which can be used to assert the value or obtained result. </ul> 
...
### 4. Cleanup:
<ul> Close all the references that are opened while setting up the test case. For this just drag and drop the Close API in the Block diagram from the SLL Drona palette and give the Launcher VI reference and application reference that need to be closed. </ul>
